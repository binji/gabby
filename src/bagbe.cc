...
State::State(GB& gb) {
  ...
  sramp = sram;
  ime = false;
  std::pair<size_t, u8> init_io[] = {{DIV, 0xac},  {NR11, 0x80}, {NR12, 0xf3},
                                     {NR14, 0x80}, {NR50, 0x77}, {NR51, 0xf3},
                                     {NR52, 0xf1}, {LCDC, 0x91}, {BGP, 0xfc},
                                     {OBP0, 0xff}, {OBP1, 0xff}, {IF, 0x01}};
  for (auto p : init_io) {
    io[p.first] = p.second;
  }
}
...
void GB::di() {
  switch (s.op_tick) {
    case 4: s.ime_delay = s.ime = false; s.op_tick = 0; break;
  }
}

void GB::ei() {
  switch (s.op_tick) {
    case 4: s.ime_delay = true; s.op_tick = 0; break;
  }
}
...
void GB::reti() {
  switch (s.op_tick) {
    case 4: s.ime_delay = false; s.ime = true;
    case 7: s.z = ReadU8(s.sp++); break;
    case 11: s.w = ReadU8(s.sp++); break;
    case 12: s.pc = s.wz; break;
    case 16: s.op_tick = 0; break;
